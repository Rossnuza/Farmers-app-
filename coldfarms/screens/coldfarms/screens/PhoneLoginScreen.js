...
const PhoneLoginScreen = (props) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Call the onLogin handler from App.js
      if (onLogin) {
        onLogin(response.farmer);
      }
    } catch (error) {
  ...
  const handleRegister = async () => {
    if (!name || !phoneNumber || !region || !city) {
        ...
        <View style={styles.pickerContainer}>
          ...
          >
            ...
            {regionOptions.map((regionOption) => (
              <Picker.Item key={regionOption} label={regionOption} value={regionOption} />
            ))}
          </Picker>
        </View>
      </View>
